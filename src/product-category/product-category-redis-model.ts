...
export interface IProductCategoryRedisModel  {
  id: string;
  title: string;
  slug: string;
  clientId: string;
...
@Injectable()
@McmsDi({
  id: 'ProductCategoryRedisModel',
  type: 'service'
})
export class ProductCategoryRedisModel extends BaseRedisModel {
  defaultSort = 'title';
  defaultWay: 'ASC' | 'DESC' = 'ASC'
  schema() {
    return  new Schema('productCategory', {
  ...
  async add(item: IProductCategoryRedisModel) {
    ...
    try {
      return await repo.save(item.id, item as unknown as Entity)  as unknown as IProductCategoryRedisModel;
    }
    catch (e) {
