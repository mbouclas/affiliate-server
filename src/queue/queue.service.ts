...
export class QueueService {
  ...
  public static queueName = process.env.ENV === 'production' ? `${process.env.DEFAULT_QUEUE_NAME}` : `${process.env.DEFAULT_QUEUE_NAME}Dev`;
  ...
  async onModuleInit() {
    ...
    QueueService.queueEvents.on('completed', (job) => {
      QueueService.logger.log(`${QueueService.queueName}: ${job.jobId}  completed`);
    });
  ...
  composeJobName(clientId: string, jobName: string) {
    return `${clientId}-${QueueService.jobEventName}${jobName}`;
  }
  ...
  public async getRepeatableJob(jobName: string, statuses?: JobType[]) {
    const jobs = await this.getRepeatableJobs();
    return jobs.find(j => j.name.indexOf(jobName) !== -1);
  }
